...
class SignalAggregator:
    ...
    def aggregate_signals(
        ...
        try:
            ...
            for symbol, symbol_signals in signals_by_symbol.items():
                ...
                total_strategies = len(set(s["strategy_name"] for s in symbol_signals))
                if total_strategies < min_strategies:
                    logger.debug(
                        f"Skipping {symbol}: only {total_strategies} strategies (min: {min_strategies})"
                    )
                    continue
                ...
                aggregated_sell = (
                    aggregation_func(sell_signals) if sell_signals else None
                )

                # Determine final signal
                final_signal = self._determine_final_signal(
                    aggregated_buy, aggregated_sell
                )
                ...
                if final_signal:
                    aggregated_signals[symbol] = {
                        ...
                        "contributing_strategies": list(
                            set(s["strategy_name"] for s in symbol_signals)
                        ),
            ...
            logger.info(
                f"Aggregated signals for {len(aggregated_signals)} symbols using {method} method"
            )
    ...
    def _majority_vote_aggregation(
        ...
        majority_signals = [
            sig_type for sig_type, count in signal_counts.items() if count == max_votes
        ]
        ...
        majority_signals_data = [
            s for s in signals if s["signal_type"] == majority_type
        ]

        avg_strength = statistics.mean([s["strength"] for s in majority_signals_data])
        avg_price = statistics.mean(
            [s["price"] for s in majority_signals_data if s["price"]]
        )
    ...
    def _determine_final_signal(
        self, buy_signal: Dict[str, Any] = None, sell_signal: Dict[str, Any] = None
    ) -> Optional[Dict[str, Any]]:
        ...
        if buy_score > sell_score + threshold:
            ...
            return {
                ...
                "confidence": 0.5,
                "contributing_signals": buy_signal["contributing_signals"]
                + sell_signal["contributing_signals"],
    ...
    def get_aggregated_signals_by_strength(
        ...
        for symbol, signal_data in aggregated_signals.items():
            ...
            if (
                final_signal
                and final_signal["strength"] >= min_strength
                and final_signal["confidence"] >= min_confidence
                and final_signal["signal_type"] != "hold"
            ):
                filtered_signals.append(
                    {"symbol": symbol, **final_signal, "metadata": signal_data}
                )

        # Sort by combined score (strength * confidence)
        filtered_signals.sort(
            key=lambda s: s["strength"] * s["confidence"], reverse=True
        )

        return filtered_signals

    def mark_signals_processed(self, aggregated_signals: Dict[str, Dict[str, Any]]):
    ...
    def get_signal_statistics(self, days_back: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get recent signals
            all_signals = self.signal_repo.list(
                limit=10000
            )  # Large limit to get all recent
            recent_signals = [
                s
            ...
            return {
                ...
                "most_active_symbols": dict(
                    sorted(by_symbol.items(), key=lambda x: x[1], reverse=True)[:10]
                ),
                "average_signals_per_day": total_signals / days_back
                if days_back > 0
                else 0,
            }
    ...
    async def aggregate_strategy_results(
