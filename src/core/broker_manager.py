...
class BrokerManager:
    ...
    def _load_broker_configurations(self):
        ...
        try:
            ...
            logger.error(f"Failed to load broker configurations: {e}")
            raise
    ...
    def _get_broker_adapter_class(self, broker_name: str):
        ...
        try:
            ...
            logger.error(f"Failed to load broker adapter for {broker_name}: {e}")
            raise
    ...
    async def connect_broker(self, broker_name: str) -> bool:
        ...
        try:
            if broker_name not in self.available_brokers:
                raise ValueError(f"Broker {broker_name} is not configured or enabled")
            ...
            if broker_info["adapter_class"] is None:
                broker_info["adapter_class"] = self._get_broker_adapter_class(
                    broker_name
                )
            ...
            if await adapter.connect():
                ...
                logger.info(
                    f"✅ Successfully connected to {broker_name} (now active broker)"
                )
                return True
            else:
    ...
    async def disconnect_broker(self, broker_name: Optional[str] = None):
        ...
        if self.active_broker is not None:
            try:
                ...
                logger.error(f"Error disconnecting from {self.active_broker_name}: {e}")

    async def disconnect_all_brokers(self):
        """Disconnect from the active broker (same as disconnect_broker)"""
        await self.disconnect_broker()

    def get_broker(self, broker_name: Optional[str] = None) -> Optional[BrokerAdapter]:
    ...
    def is_broker_connected(self, broker_name: Optional[str] = None) -> bool:
        ...
        return [
            {
                ...
                "connected": True,
                "paper_trading": broker_info.get("config", {}).get(
                    "paper_trading", True
                ),
    ...
    async def place_order(self, order_request: OrderRequest) -> OrderResponse:
        ...
        if not self.active_broker:
            raise RuntimeError("No broker is currently connected")
...
async def initialize_default_brokers():
    ...
    logger.info(
        f"Attempting to connect to first available broker from: {default_brokers}"
    )
