...
from db.repository import SymbolRepository, StockDataRepository
...
class EnhancedStockScreener:
    ...
    async def run_comprehensive_screening(
        ...
        try:
            ...
            all_candidates = pd.concat(
                [screener_data["gainers"], screener_data["losers"]], ignore_index=True
            )
            ...
            screening_results = await self._score_and_rank_candidates(
                filtered_candidates, criteria
            )
            ...
            logger.info(
                f"Screening completed: {len(screening_results)} stocks identified"
            )
    ...
    def _apply_advanced_filters(
        self, candidates: pd.DataFrame, criteria: ScreeningCriteria
    ) -> pd.DataFrame:
        ...
        # Change filters
        filtered = filtered[
            (filtered["change"] >= criteria.min_daily_change)
            & (filtered["change"] <= criteria.max_daily_change)
        ]
    ...
    async def _score_and_rank_candidates(
        ...
        for _, row in candidates.iterrows():
            try:
                ...
                logger.warning(
                    f"Error scoring candidate {row.get('symbol', 'unknown')}: {e}"
                )
                continue
    ...
    def _calculate_candidate_score(
        self, row: pd.Series, criteria: ScreeningCriteria
    ) -> float:
    ...
    async def get_prediction_enhanced_screening(
        ...
        try:
            ...
            for result in basic_results:
                try:
                    ...
                    if result.daily_change_percent < -2.0:  # Significant loser
                        future_gainers = predictor.get_losers_to_gainers(
                            losers_to_scan=10, gain_ratio=5.0, future_periods=5
                        )
                    ...
                    logger.warning(
                        f"Error enhancing prediction for {result.symbol}: {e}"
                    )
                    enhanced_results.append(result)
            ...
            logger.info(
                f"Prediction enhancement completed for {len(enhanced_results)} candidates"
            )
            return enhanced_results

        except Exception as e:
            logger.error(f"Error in prediction-enhanced screening: {e}")
            raise

    async def run_automated_screening(
        self, criteria: ScreeningCriteria, interval_minutes: int = 60
    ) -> None:
        ...
        while True:
            try:
                ...
                if await self._is_market_open():
                    ...
                    logger.info(
                        f"Automated screening completed: {len(results)} stocks identified"
                    )
                else:
                ...
                logger.error(f"Error in automated screening: {e}")
    ...
    async def _process_screening_results(self, results: List[ScreeningResult]):
        ...
        try:
            ...
            logger.error(f"Error processing screening results: {e}")

    async def _update_symbol_metadata(self, symbol: str, metadata: Dict[str, Any]):
    ...
    def get_screening_results(self, symbol: str = None) -> Dict[str, ScreeningResult]:
    ...
    def add_custom_screening_criteria(self, name: str, criteria: ScreeningCriteria):
    ...
    def get_screening_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "top_scored_symbols": [
                ...
                for r in sorted(
                    self.screening_results.values(), key=lambda x: x.score, reverse=True
                )[:10]
            ],
        }
